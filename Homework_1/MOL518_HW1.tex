...
\begin{enumerate}[label=\alph*)]
  \item In a single code cell, define the following variables (with these exact names) in Python:
  \begin{itemize}
  ...
  \item Compute the number of infectious units needed for this infection (call it \texttt{iu\_needed}). Briefly explain (in a Markdown sentence) what MOI means in words.
  ...
  \item Print a short, readable summary that includes: \texttt{cells}, \texttt{moi}, \texttt{iu\_needed}, virus volume ($\mu$L), media volume ($\mu$L).
\end{enumerate}
...
\begin{enumerate}[label=\alph*)]
  \item Load \texttt{Lecture\_2/data/growth\_curve1.csv} as a 2D NumPy array called \texttt{data}. (Include the exact code you used.)
  \item Extract \texttt{time} (seconds) and \texttt{od} (unitless) as 1D arrays. In one Markdown sentence, state what each variable represents.
  \item Print the shape of \texttt{data}. Then print the first row and the last row, and briefly interpret what those rows mean.
  \item Compute and print the sampling interval in seconds: \texttt{dt\_sec = time[1] - time[0]}. In one sentence, explain what it would suggest if \texttt{dt\_sec} changed over time.
