...
You ran a plate reader growth experiment and saved the results as a CSV file. Before doing any serious analysis, you want to do a fast sanity check to confirm the file saved correctly, the sampling looks consistent, and the values are in a reasonable range. {\it Do not use AI to generate code for this problem. You may use AI to debug if you get an error}

\begin{enumerate}[label=\alph*)]
  \item Load \texttt{Lecture\_2/data/growth\_curve1.csv} as a 2D NumPy array called \texttt{data}.
  \item Print the shape of \texttt{data}. Describe what this tell you. 
  \item Print the first row and the last row, and briefly describe what those rows mean.
  \item Extract \texttt{time} in seconds and \texttt{od} as 1D arrays.
  \item Compute and print the sampling interval, the time between consecutive timepoints, in minutes.
  \item Compute and print the total duration of the experiment in units of days.
  \item Compute and print the following summary quantities, and label each one clearly in the output:
  \begin{itemize}
    \item absolute OD change from start to end
    \item total fold change in OD from start to end
  \end{itemize}
\end{enumerate}

\section*{Problem 4: Fake data!}

In this problem, you will generate a small \emph{synthetic} growth curve dataset. ``Fake'' data can be very useful for testing code, practicing analysis, or simulating experiments because you know exactly what to expect. {\it Do not use AI to generate code for this problem. You may use AI to debug if you get an error}

\begin{enumerate}[label=\alph*)]
  \item Create a NumPy array called \texttt{time\_hr} with time points in hours the go from 0 to 12 hours in increments of 0.25 hours. Print \texttt{time\_hr} to check that it looks correct.
  \item Define two variables: \texttt{od0 = 0.08} and \texttt{doubling\_time\_hr = 1.5}. Explain what ``doubling time'' means in the context of bacterial growth.
  \item Compute the \texttt{od} time series using the following fomula
  \begin{equation}
  OD = OD_0 \times 2^\frac{t}{t_{doubling}}.
  \end{equation}
  Print \texttt{od} to confirm your results.
  \item Normalize \texttt{od} by its first value. Print \texttt{od\_norm}.
  \item Use \texttt{np.column\_stack} to make a 2D array with columns \texttt{time\_hr}, \texttt{od}, \texttt{od\_norm}. Print the resulting array and its shape.
  \item Save the result to \texttt{simulated\_growth.csv}.
  \item Reload the file you saved into a varaible \texttt{data2}. Print \texttt{data2.shape} and the first 3 rows, and confirm that the reloaded numbers match what you expect.
\end{enumerate}

\section*{Problem 5: Sample Tracking and Lane Batching}

You will often need to organize samples into a workable datastructure for downstream analysis. In this problem, you will treat a Python list as your ``sample sheet'' and practice counting, appending, sorting, and slicing into batches. {\it Do not use AI to generate code for this problem. You may use AI to debug if you get an error}

\begin{enumerate}[label=\alph*)]
  \item Create a Python list called \texttt{samples} with the following sample IDs, in this exact order:
...
\end{lstlisting}
  ...
  \item Print the number of samples using the list and briefly interpret what that number means in this context.
  \item Append two new samples to the end of the list: \texttt{"Blank"} and \texttt{"PositiveCtrl"}. Print the updated list.
  \item Create a new list called \texttt{samples\_sorted} that is an alphabetically sorted version of \texttt{samples}. Print both lists (original and new) to demonstrate that only the new one was sorted.
  ...
  \end{itemize}
  \item Print \texttt{lane1}, \texttt{lane2}, and \texttt{lane3}. Do you think slicing a safer approach than manually retyping sample IDs? What if your list had 1000 samples?
\end{enumerate}
