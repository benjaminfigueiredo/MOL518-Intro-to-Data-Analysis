...
\begin{enumerate}[label=\alph*)]
  ...
  \item For each filename, split the stem on underscores (\texttt{\_}) and extract the strain (the second field in the filename).
...
\begin{enumerate}[label=\alph*)]
  ...
  \item Create a 2x3 grid of histograms for bin counts \texttt{[8, 20, 40, 60, 120, 200]}. Use \texttt{plt.subplots} and a \texttt{for} loop to fill the panels.
